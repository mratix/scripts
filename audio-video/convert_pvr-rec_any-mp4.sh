...
case "$profile" in
    480p)
        # draft profile integrated: 480p mpeg4+aac
        ...
        ;;
    720p)
        # draft profile integrated: 720p x264+aac
        ...
        ;;
    1080p)
        # draft profile integrated: 1080p x265+aac
...
for item in "$srcdir"/*; do
    ...
    if ffmpeg -hide_banner -nostats -loglevel error -fflags +genpts+igndts -i "$item" "${optenc[@]}" "${opts[@]}" "$outfile"; then
        ...
        ((errors++))
        echo "[!] ffmpeg failed: $item"
    fi
done

if (( converted == 0 && skipped == 0 )); then
    echo "[ ] No matching videofiles in source directory."
fi
